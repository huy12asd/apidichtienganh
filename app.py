...
def build_regex_and_dict():
    ...
    try:
        ...
        for term in terms_data:
            ...
            term_list.append(re.escape(term_key))
...
# --- BIẾN TOÀN CỤC (tải 1 lần lúc khởi động) ---
...
# --- HÀM DỊCH CÓ CACHE + CHIA BATCH NHỎ ---
def translate_with_cache(texts, batch_size=50):
    """
    Dịch danh sách `texts`:
    - Sử dụng cache để tránh dịch lại.
    - Dịch các mục chưa cache theo batch_size để tránh timeout/rate-limit.
    - Nếu batch lỗi, fallback về nguyên bản (tránh crash).
    Trả về danh sách kết quả theo đúng thứ tự của `texts`.
    """
    results = []
    # Danh sách các text chưa cache
    uncached = [t for t in texts if t not in translation_cache]

    # Dịch uncached theo từng batch nhỏ
    for i in range(0, len(uncached), batch_size):
        ...
        try:
            ...
            # Nếu lỗi, log và fallback: lưu text gốc vào cache để tránh gọi lại
            print(f"[ERROR] Google Translate batch failed: {e} - fallback to original for this batch.")
            for k in batch:
                translation_cache[k] = k
...
def api_translate():
    ...
    # --- 2. Chuẩn bị gửi Google (chỉ những đoạn có chữ latin) ---
    texts_to_google = []
    ...
    # --- 3. Gọi dịch (dùng translate_with_cache với batch nhỏ) ---
    google_translated_results = []
    if texts_to_google:
        print(f"[API] *** ĐANG GỌI GOOGLE DỊCH cho {len(texts_to_google)} đoạn (theo batch nhỏ)... ***")
        start_time_google = time.time()
        try:
            google_translated_results = translate_with_cache(texts_to_google, batch_size=50)
            print(f"[API] *** GOOGLE DỊCH HOÀN TẤT sau {time.time() - start_time_google:.4f}s ***")
        except Exception as e:
            print(f"[API] LỖI NGOẠI LỆ (translate_with_cache): {e} - Trả về text gốc.")
            google_translated_results = texts_to_google
    ...
    for i, text in enumerate(preprocessed_texts):
        ...
        final_translations.append({"type": item_type, "content": final_content})
...
# --- Endpoint để reload từ điển (khi bạn update DB không cần restart server) ---
@app.route("/api/reload_terms", methods=["GET"])
def reload_terms():
    global COMPILED_REGEX, TERMS_DICT
    COMPILED_REGEX, TERMS_DICT = build_regex_and_dict()
    return jsonify({"status": "ok", "count": len(TERMS_DICT)})
