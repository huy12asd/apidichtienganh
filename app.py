...
def build_regex_and_dict():
    ...
    try:
        ...
        for term in terms_data:
            ...
            # Tạo HTML cho tooltip (TRÊN MỘT DÒNG - đã xóa \n)
...
def api_translate():
    ...
    for i, text in enumerate(preprocessed_texts):
        final_content = ""
        item_type = "plain" # Mặc định là văn bản thô
        
        if i in google_map:
            # Dùng text đã dịch của Google
            translated_text = google_map[i]
            # Hậu xử lý: Thay thế placeholder [[...]] bằng HTML
            final_content = re.sub(r'\[\[(.*?)\]\]', r'\1', translated_text)
            
            # *** BƯỚC SỬA LỖI: ***
            # Kiểm tra xem kết quả cuối cùng có chứa HTML không
            if "<span style=" in final_content:
                item_type = "html"
            ...
            if text.strip(): 
                ...
                item_type = "html" # Đây luôn là HTML từ từ điển
            else:
                final_content = ""
                # item_type = "plain" (đã mặc định)
